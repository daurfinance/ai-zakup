...
let LotsService = class LotsService {
    ...
    async notifyInterestedSuppliers(lot) {
    ...
    async logTenderAction(userId, action, tenderId, details) {
    ...
    async update(userId, id, updateLotDto) {
        ...
        if (updateLotDto.deadlines) {
            this.validateTenderData({ ...lot, ...updateLotDto });
        }
        const updatedLot = await this.prisma.lot.update({
            where: { id },
            data: { ...updateLotDto, deadlines: updateLotDto.deadlines, criteria: updateLotDto.criteria, docs: updateLotDto.docs, contactInfo: updateLotDto.contactInfo, deliveryTerms: updateLotDto.deliveryTerms, paymentTerms: updateLotDto.paymentTerms },
            include: {
                customer: true,
            },
        });
    ...
    async cancel(userId, id, reason) {
        ...
        if (!['draft', 'published'].includes(lot.status)) {
            throw new common_1.BadRequestException('Нельзя отменить тендер в текущем статусе');
        }
    ...
    async notifyTenderCancellation(lot, reason) {
